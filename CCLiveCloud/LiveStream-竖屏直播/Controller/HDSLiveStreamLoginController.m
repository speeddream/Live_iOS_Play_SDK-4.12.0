//
//  HDSLiveStreamLoginController.m
//  CCLiveCloud
//
...
@interface HDSLiveStreamLoginController ()<UITextFieldDelegate,RequestDataDelegate>
...
@implementation HDSLiveStreamLoginController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //设置导航栏信息
    self.navigationItem.title = LOGIN_LIVE;
    ...
    //设置输入框和登陆按钮
    self.textFieldUserId.text = GetFromUserDefaults(WATCH_LIVE_USERID);//userId
    self.textFieldRoomId.text = GetFromUserDefaults(WATCH_LIVE_ROOMID);//roomId
    self.textFieldUserName.text = GetFromUserDefaults(WATCH_LIVE_USERNAME);//userName
    self.textFieldUserPassword.text = GetFromUserDefaults(WATCH_LIVE_PASSWORD);//password
...
-(void)loginSucceedPlay {
    SaveToUserDefaults(WATCH_LIVE_USERID,_textFieldUserId.text);
    SaveToUserDefaults(WATCH_LIVE_ROOMID,_textFieldRoomId.text);
    SaveToUserDefaults(WATCH_LIVE_USERNAME,_textFieldUserName.text);
    SaveToUserDefaults(WATCH_LIVE_PASSWORD,_textFieldUserPassword.text);
    [_loadingView removeFromSuperview];
    _loadingView = nil;
    [UIApplication sharedApplication].idleTimerDisabled=YES;
    HDSLiveStreamController *liveShopVC = [[HDSLiveStreamController alloc] initWithRoomName:self.roomName];
    liveShopVC.modalPresentationStyle = 0;
    liveShopVC.screenCaptureSwitch = YES;
    [self presentViewController:liveShopVC animated:YES completion:^{
    }];
}
...
-(void)onSweepCode {
    ...
    switch (status) {
        ...
        case AVAuthorizationStatusAuthorized:{
            // 已经开启授权，可继续
            ScanViewController *scanViewController = [[ScanViewController alloc] initWithType:5];
...
-(TextFieldUserInfo *)textFieldUserId {
    if(_textFieldUserId == nil) {
        ...
        [_textFieldUserId textFieldWithLeftText:LOGIN_TEXT_USERID placeholder:LOGIN_TEXT_USERID_PLACEHOLDER lineLong:YES text:GetFromUserDefaults(WATCH_LIVE_USERID)];
...
-(TextFieldUserInfo *)textFieldRoomId {
    if(_textFieldRoomId == nil) {
        ...
        [_textFieldRoomId textFieldWithLeftText:LOGIN_TEXT_ROOMID placeholder:LOGIN_TEXT_ROOMID_PLACEHOLDER lineLong:NO text:GetFromUserDefaults(WATCH_LIVE_ROOMID)];
...
-(TextFieldUserInfo *)textFieldUserName {
    if(_textFieldUserName == nil) {
        ...
        [_textFieldUserName textFieldWithLeftText:LOGIN_TEXT_USERNAME placeholder:LOGIN_TEXT_USERNAME_PLACEHOLDER lineLong:NO text:GetFromUserDefaults(WATCH_LIVE_USERNAME)];
...
-(TextFieldUserInfo *)textFieldUserPassword {
    if(_textFieldUserPassword == nil) {
        ...
        [_textFieldUserPassword textFieldWithLeftText:LOGIN_TEXT_PASSWORD placeholder:LOGIN_TEXT_PASSWORD_PLACEHOLDER lineLong:NO text:GetFromUserDefaults(WATCH_LIVE_PASSWORD)];
