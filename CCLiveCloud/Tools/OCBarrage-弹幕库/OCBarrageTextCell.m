...
//    CGRect textFrame1 = [self.textDescriptor.attributedText.string boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:[self.textDescriptor.attributedText attributesAtIndex:0 effectiveRange:NULL] context:nil];
    CGRect textFrame = [self.textDescriptor.attributedText boundingRectWithSize:CGSizeMake(100000, CGFLOAT_MAX)
                                            options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                            context:nil];
    UIFont *font = self.textLabel.font;
    CGSize size = [@"测试" sizeWithAttributes:@{
                                              NSFontAttributeName : font
                                              }];
//    NSLog(@"size = %@", NSStringFromCGSize(size));
//    if (textFrame1.size.height - size.height > 5) {
        self.textLabel.frame = CGRectMake(0, 0, textFrame.size.width, size.height);
//    NSLog(@"__test textLabel%@", self.textLabel);
//    }else{
//        self.textLabel.frame = CGRectMake(0, 0, textFrame.size.width, textFrame1.size.height);
//    }
}
