...
@interface HDRedPacketRainView ()<HDSRedPacketViewDelegate>
...
@implementation HDRedPacketRainView

// MARK: - API
- (instancetype)initWithFrame:(CGRect)frame configuration:(HDRedPacketRainConfiguration *)configuration tapRedPacketClosure:(TagRedPacketClosure)tagRedPacketClosure {
...
- (void)initRedPacket {
    ...
    for (int j = 0; j < 12; j++) {
        ...
        HDSRedPacketView *redPacket = [[HDSRedPacketView alloc] initWithFrame:CGRectMake([self getRandomX], -(self.itemOffset * j + self.itemH), self.itemW, self.itemH)];
    ...
    @autoreleasepool {
        dispatch_async(dispatch_get_main_queue(), ^{
            for (int i = 0; i < self.redPacketSourceArray.count; i++) {
                HDSRedPacketView *redPacket = self.redPacketSourceArray[i];
...
- (void)hdsViewDidTouch:(HDSRedPacketView *)touchView {
    @autoreleasepool {
        ...
        imageView.animationImages = [self.openImageArray copy];
